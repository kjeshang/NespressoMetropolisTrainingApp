...
# Import the data -----------------------------------------
df = pd.read_csv("PreparedCoffeeData.csv", index_col=False);
...
def get_validationContent(techniqueSelected, df, min_df, max_df, max_features, stop_words, n_lower, n_upper, sublinear_tf, analyzer, token_pattern, test_size, random_state, target, alpha):
    if techniqueSelected == 0:
...
def get_mainContent(coffeeSelectValue, machineTypeValue, servingValue, includeDecafValue, techniqueSelected, numRec, min_df, max_df, max_features, ngram_range, sublinear_tf, analyzer, similarityMeasure, test_size, random_state, target, alpha):
    if includeDecafValue == "Yes":
        mask = (df["Type"].isin(machineTypeValue)) & (df["Serving"].isin(servingValue));
    else:
        mask = (df["Type"].isin(machineTypeValue)) & (df["Serving"].isin(servingValue)) & (df["Decaf Coffee?"] != "Yes");
