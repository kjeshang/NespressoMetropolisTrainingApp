...
def getPipeAccuracy(dataframe, features, target, vectorizer, test_size, random_state, alpha):
    ...
    cr = classification_report(y_test, y_predPipe, zero_division=0, output_dict=True);
    df_cr = pd.DataFrame(cr).transpose().round(3);

    data = confusion_matrix(y_test, y_predPipe);
    ...
    data = [[pipeScore]];
    df_score = pd.DataFrame(data, columns=['Pipe Score']);
